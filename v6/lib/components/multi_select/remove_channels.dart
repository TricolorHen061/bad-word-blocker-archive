...
Future<void> removeChannelsMultiSelectHandler(
    IMultiselectInteractionEvent event, Guild guild) async {
  final channelIdsToRemove = event.interaction.values;
  final bypasses = await getGuildBypasses(guild.id);
  for (final channelId in channelIdsToRemove) {
    bypasses.channels.remove(channelId);
  }
  ...
  final nextSelectMenuOptions = previousSelectMenu.options
      .filter((t) => channelIdsToRemove.notElem(t.value))
      .map((e) => MultiselectOptionBuilder(e.label, e.value));
  final messageBuilder = ComponentMessageBuilder()
    ..embeds = [
      EmbedBuilder()
        ..title = "Channels Successfully Removed"
        ..description =
            "$previousDescription\n**- ${channelIdsToRemove.length} channels(s) removed**"
        ..color = greenEmbedColor
    ]
    ..componentRows = [
      ComponentRowBuilder()
        ..addComponent(
            MultiselectBuilder("remove_channels", nextSelectMenuOptions)
  ...
  if (nextSelectMenuOptions.isNotEmpty) {
    ...
    final messageBuilder = ComponentMessageBuilder()
      ..embeds = [
        EmbedBuilder()
          ..title = "All Bypass Channels Removed"
          ..description = "All bypass channels have been removed."
          ..color = greenEmbedColor
      ]
      ..componentRows = [];
    await event.respond(messageBuilder, hidden: true);
